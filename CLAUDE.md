...
- **`config.py`** — Runtime constants: desktop container name, network name, display settings, provider config (model names, API settings, per-provider reasoning `effort`), agent loop parameters, and `ASSESSMENT_FORMAT` (the assessment template shared by prompts and providers). `ANTHROPIC_BACKEND` selects `api` (first-party) vs `bedrock` (Amazon Bedrock); most values are env-overridable. No project-specific config.
